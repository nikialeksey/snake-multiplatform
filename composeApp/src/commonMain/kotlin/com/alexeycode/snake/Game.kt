...
fun Game(direction: Pair<Int, Int>) {
    var fieldSizePx by remember { mutableStateOf(0 to 0) }
    ...
    LaunchedEffect(snakePosition, fieldSizePx) {
        delay(0.2.seconds)
    ...
    Canvas(
        modifier = Modifier
            .fillMaxSize()
            .onSizeChanged {
                fieldSizePx = it.width to it.height
            }
    ...
    FIELD_WIDTH = toDp(fieldSizePx.first)
    FIELD_HEIGHT = toDp(fieldSizePx.second)
}
